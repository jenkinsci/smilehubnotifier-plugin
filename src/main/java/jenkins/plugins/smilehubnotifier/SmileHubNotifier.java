package jenkins.plugins.smilehubnotifier;
...
public class SmileHubNotifier extends Notifier {

  private static final Logger LOGGER = Logger.getLogger(SmileHubNotifier.class.getName());
  ...
  public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
    ...
    for (Publisher publisher : map.values()) {
      if (publisher instanceof SmileHubNotifier) {
        LOGGER.info("Invoking Completed...");
        new ActiveNotifier((SmileHubNotifier) publisher, listener).completed(build);
      }
    }
    return true;
  }
    ...
    if (notifyStart) {
      ...
      for (Publisher publisher : map.values()) {
        if (publisher instanceof SmileHubNotifier) {
          ...
          new ActiveNotifier((SmileHubNotifier) publisher, listener).started(build);
        }
      }
    }
  ...
  public static class DescriptorImpl extends BuildStepDescriptor<Publisher> {
    ...
    public FormValidation doTestConnection(@QueryParameter("rocketServerUrl") final String rocketServerUrl,
      ...
      try {
        ...
        String message = "SmileHub/Jenkins plugin: you're all set on " + targetBuildServerUrl;
